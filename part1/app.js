...
(async () => {
    ...
    `);
      ...
      `);
        ...
        ((SELECT dog_id FROM Dogs WHERE name = 'Tim'), '2025-04-10 11:00:00', 35, 'Parsley Road', 'completed');
      `);
        }
        ...
        VALUES
        ((SELECT request_id FROM WalkRequests WHERE status = 'completed' AND dog_id = (SELECT dog_id FROM Dogs WHERE name = 'Tim') LIMIT 1),
    ...
    try {
        const [walkers] = await db.execute(`
            SELECT
                ...
                ROUND(AVG(rating.rating), 1) AS averageRating,
                COUNT(DISTINCT walkRequest.request_id) AS totalNumberOfWalks
            FROM Users user
